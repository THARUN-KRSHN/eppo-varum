...
import { Check, LoaderCircle, MapPin, Save, ShieldCheck, TriangleAlert, X } from 'lucide-react';
...
export default function Verify() {
  ...
  useEffect(() => {
    const id = localStorage.getItem('eppo-varum-last-document') || localStorage.getItem('transitlens-last-document');
  ...
  useEffect(() => {
    ...
    fetch(`${api}/documents/${documentId}/extraction`, { headers: { Authorization: `Bearer ${token}` } }).then(response => response.json()).then(result => {
      if (result.detail) setMessage(result.detail); else { setData(result); setLocationQuery(result.stop_location?.name || ''); }
  ...
  const saveCorrection = async (index: number, field: string, value: any) => {
    ...
    if (!response.ok) setMessage(result.detail || 'Correction could not be saved.'); else { setData(result.data); setMessage(copy.verify.saved); }
  };
  ...
  const publish = async () => {
    ...
    const result = await response.json(); if (!response.ok) setMessage(result.detail || 'Publishing failed.'); else setModal(true);
  };
  ...
  return <main className="mx-auto max-w-6xl px-5 pb-28 pt-32 md:pt-40">
    ...
    <section className="surface mt-10 p-6 md:p-8">
      ...
      <div className="mt-7 overflow-hidden rounded-2xl border border-green-100"><div className="grid grid-cols-[1fr_112px_112px_82px] gap-3 bg-green-50 px-4 py-3 text-xs font-bold uppercase tracking-wider text-green-700"><span>Stop</span><span>Arrival</span><span>Departure</span><span>Save</span></div>{data.stops.map((stop: any, index: number) => <div key={stop.sequence} className="grid grid-cols-[1fr_112px_112px_82px] items-center gap-3 border-t border-green-100 px-4 py-4"><div><p className="font-bold text-green-950">{stop.name_en}</p><p className="text-xs text-green-950/55">{stop.name_ml || 'Malayalam name not available'}</p></div><input aria-label={`Arrival time for ${stop.name_en}`} type="time" value={stop.arrival_time?.slice(0, 5) || ''} onChange={event => update(index, 'arrival_time', event.target.value)} onBlur={event => saveCorrection(index, 'arrival_time', event.target.value)} className="w-full rounded-xl border border-green-100 px-2 py-2 font-bold outline-none focus:border-green-500" /><input aria-label={`Departure time for ${stop.name_en}`} type="time" value={stop.departure_time?.slice(0, 5) || ''} onChange={event => update(index, 'departure_time', event.target.value)} onBlur={event => saveCorrection(index, 'departure_time', event.target.value)} className="w-full rounded-xl border border-green-100 px-2 py-2 font-bold outline-none focus:border-green-500" /><button aria-label="Save timetable row" onClick={() => { saveCorrection(index, 'arrival_time', stop.arrival_time); saveCorrection(index, 'departure_time', stop.departure_time); }} className="grid h-10 w-10 place-items-center rounded-xl bg-green-100 text-green-700"><Save size={17} /></button></div>)}</div>
      {message && <p className="mt-4 text-sm font-semibold text-green-700">{message}</p>}<button onClick={publish} className="mt-7 inline-flex min-h-12 items-center gap-2 rounded-xl bg-green-700 px-5 font-bold text-white hover:bg-green-800"><ShieldCheck size={18} /> Verify and publish</button>
    </section>
    {modal && <div className="fixed inset-0 z-50 grid place-items-center bg-green-950/35 p-5"><div className="surface max-w-md p-8 text-center"><Check className="mx-auto text-green-600" size={40} /><h2 className="mt-4 text-2xl font-bold text-green-950">Published</h2><p className="mt-2 text-green-950/60">The verified timetable is now available at this bus stop.</p><button onClick={() => setModal(false)} className="mt-6 rounded-xl bg-green-700 px-5 py-3 font-bold text-white">Close</button></div></div>}
  </main>;
}
