...
The backend is structured around the finalized eppo varum (എപ്പോ വരും) flow: upload → preprocessing → OpenRouter vision/OCR → table/semantic extraction → normalization → confidence → human verification → publication → map/search/alerts/GTFS.
